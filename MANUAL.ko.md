...
설정 검색 순서: 현재 디렉토리 (`.qlaude/`) → 홈 디렉토리 (`~/.qlaude/`) → 기본값.
...
`.qlaude/telegram.json`을 편집하여 커스터마이즈합니다:
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `enabled` | `false` | 텔레그램 알림 활성화 |
| `botToken` | — | 텔레그램 Bot API 토큰 |
| `chatId` | — | 대상 채팅 ID |
| `language` | `"en"` | 메시지 언어: `"en"` (영어) 또는 `"ko"` (한국어) |
| `confirmDelayMs` | `30000` | 멀티 인스턴스 폴링 시 업데이트 확인 지연 시간 (ms) |
| `messages` | `{}` | 개별 메시지 문자열 오버라이드 (`telegram-messages.ts` 카탈로그 키 사용) |
| `templates` | `{}` | 알림 유형별 레이아웃 템플릿 |
...
| `[BP]` | 중단점 (`>>#`) |
| `[ML]` | 멀티라인 프롬프트 |
| `[LABEL:이름]` | 세션 레이블 저장 지점 |
| `[LOAD:이름]` | 세션 로드 지점 |

태그는 조합될 수 있습니다 (예: `[ML] [NEW]`는 멀티라인 새 세션 프롬프트).
...
모든 상태 감지 패턴은 `.qlaude/patterns.json`에서 커스터마이즈할 수 있습니다. 각 패턴 카테고리를 독립적으로 설정 가능합니다.
...
다중 패턴 카테고리 (`selectionPrompt`, `interrupted`, `spinner`, `taskFailure`, `textInputKeywords`):
...
`.qlaude/telegram.json`의 `templates`에서 알림 전체 레이아웃을 오버라이드합니다. 템플릿은 `{변수}` 플레이스홀더를 사용합니다.
...
| `{header}` | 앱 헤더 | `🤖 *[qlaude]*` |
| `{emoji}` | 이벤트 이모지 | `⚠️`, `✅`, `❌` 등 |
| `{title}` | 이벤트 제목 (볼드) | `*입력 필요*` |
| `{hostInfo}` | 호스트명 + IP | `🖥️ myhost (192.168.1.1)` |
| `{instanceInfo}` | 인스턴스 ID | `🆔 myhost:12345` |
| `{project}` | 프로젝트명 | `📁 my-project` |
| `{queue}` | 큐 항목 수 | `📋 큐: 3개 항목` |
| `{message}` | 이벤트별 메시지 | `💬 사유 텍스트` |
| `{context}` | 화면 컨텍스트 (코드 블록) | ````...```` |
| `{options}` | 파싱된 옵션 목록 | `1. 옵션 하나` |
...
**스피너 안전 일시정지 오작동**: 스피너 패턴이 실제 스피너가 아닌 화면 내용과 매칭되었습니다. `.qlaude/patterns.json`에서 스피너 패턴을 커스터마이즈하여 오감지를 제거합니다.

**텔레그램 미동작**: `.qlaude/telegram.json`의 `botToken`과 `chatId`를 확인합니다. 봇이 해당 채팅에 메시지를 보낼 권한이 있는지 확인합니다. 디버그 로그에서 텔레그램 관련 오류를 확인합니다.
